(set-info :smt-lib-version 2.6)
(set-logic QF_AUFLIA)
(set-info :source |
Benchmarks used in the followin paper:
Big proof engines as little proof engines: new results on rewrite-based satisfiability procedure
Alessandro Armando, Maria Paola Bonacina, Silvio Ranise, Stephan Schulz. 
PDPAR'05
http://www.ai.dist.unige.it/pdpar05/


|)
(set-info :category "crafted")
(set-info :status unsat)
(declare-fun a_253 () (Array Int Int))
(declare-fun a_254 () (Array Int Int))
(declare-fun a_256 () (Array Int Int))
(declare-fun a_258 () (Array Int Int))
(declare-fun a_260 () (Array Int Int))
(declare-fun a_262 () (Array Int Int))
(declare-fun a_264 () (Array Int Int))
(declare-fun a_266 () (Array Int Int))
(declare-fun a_268 () (Array Int Int))
(declare-fun a_270 () (Array Int Int))
(declare-fun a_272 () (Array Int Int))
(declare-fun a_274 () (Array Int Int))
(declare-fun a_276 () (Array Int Int))
(declare-fun a_278 () (Array Int Int))
(declare-fun a_280 () (Array Int Int))
(declare-fun a_282 () (Array Int Int))
(declare-fun a_283 () (Array Int Int))
(declare-fun a_284 () (Array Int Int))
(declare-fun a_286 () (Array Int Int))
(declare-fun a_288 () (Array Int Int))
(declare-fun a_290 () (Array Int Int))
(declare-fun a_292 () (Array Int Int))
(declare-fun a_294 () (Array Int Int))
(declare-fun a_296 () (Array Int Int))
(declare-fun a_298 () (Array Int Int))
(declare-fun a_300 () (Array Int Int))
(declare-fun a_302 () (Array Int Int))
(declare-fun a_304 () (Array Int Int))
(declare-fun e_252 () Int)
(declare-fun e_255 () Int)
(declare-fun e_257 () Int)
(declare-fun e_259 () Int)
(declare-fun e_261 () Int)
(declare-fun e_263 () Int)
(declare-fun e_265 () Int)
(declare-fun e_267 () Int)
(declare-fun e_269 () Int)
(declare-fun e_271 () Int)
(declare-fun e_273 () Int)
(declare-fun e_275 () Int)
(declare-fun e_277 () Int)
(declare-fun e_279 () Int)
(declare-fun e_281 () Int)
(declare-fun e_285 () Int)
(declare-fun e_287 () Int)
(declare-fun e_289 () Int)
(declare-fun e_291 () Int)
(declare-fun e_293 () Int)
(declare-fun e_295 () Int)
(declare-fun e_297 () Int)
(declare-fun e_299 () Int)
(declare-fun e_301 () Int)
(declare-fun e_303 () Int)
(declare-fun e_306 () Int)
(declare-fun e_307 () Int)
(declare-fun i_305 () Int)
(declare-fun a1 () (Array Int Int))
(declare-fun i0 () Int)
(declare-fun i2 () Int)
(declare-fun i3 () Int)
(declare-fun i4 () Int)
(declare-fun i5 () Int)
(declare-fun i6 () Int)
(declare-fun i7 () Int)
(declare-fun sk ((Array Int Int) (Array Int Int)) Int)
(assert (= a_253 (store a1 i3 e_252)))
(assert (= a_254 (store a_253 i3 e_252)))
(assert (= a_256 (store a_254 i5 e_255)))
(assert (= a_258 (store a_256 i7 e_257)))
(assert (= a_260 (store a_258 i6 e_259)))
(assert (= a_262 (store a_260 i7 e_261)))
(assert (= a_264 (store a_262 i5 e_263)))
(assert (= a_266 (store a_264 i7 e_265)))
(assert (= a_268 (store a_266 i0 e_267)))
(assert (= a_270 (store a_268 i7 e_269)))
(assert (= a_272 (store a_270 i4 e_271)))
(assert (= a_274 (store a_272 i2 e_273)))
(assert (= a_276 (store a_274 i5 e_275)))
(assert (= a_278 (store a_276 i2 e_277)))
(assert (= a_280 (store a_278 i7 e_279)))
(assert (= a_282 (store a_280 i0 e_281)))
(assert (= a_283 (store a_258 i7 e_261)))
(assert (= a_284 (store a_283 i6 e_259)))
(assert (= a_286 (store a_284 i7 e_285)))
(assert (= a_288 (store a_286 i5 e_287)))
(assert (= a_290 (store a_288 i7 e_289)))
(assert (= a_292 (store a_290 i0 e_291)))
(assert (= a_294 (store a_292 i2 e_293)))
(assert (= a_296 (store a_294 i4 e_295)))
(assert (= a_298 (store a_296 i2 e_297)))
(assert (= a_300 (store a_298 i5 e_299)))
(assert (= a_302 (store a_300 i7 e_301)))
(assert (= a_304 (store a_302 i0 e_303)))
(assert (= e_252 (select a1 i3)))
(assert (= e_255 (select a_254 i7)))
(assert (= e_257 (select a_254 i5)))
(assert (= e_259 (select a_258 i7)))
(assert (= e_261 (select a_258 i6)))
(assert (= e_263 (select a_262 i7)))
(assert (= e_265 (select a_262 i5)))
(assert (= e_267 (select a_266 i7)))
(assert (= e_269 (select a_266 i0)))
(assert (= e_271 (select a_270 i2)))
(assert (= e_273 (select a_270 i4)))
(assert (= e_275 (select a_274 i2)))
(assert (= e_277 (select a_274 i5)))
(assert (= e_279 (select a_278 i0)))
(assert (= e_281 (select a_278 i7)))
(assert (= e_285 (select a_284 i5)))
(assert (= e_287 (select a_284 i7)))
(assert (= e_289 (select a_288 i0)))
(assert (= e_291 (select a_288 i7)))
(assert (= e_293 (select a_292 i4)))
(assert (= e_295 (select a_292 i2)))
(assert (= e_297 (select a_296 i5)))
(assert (= e_299 (select a_296 i2)))
(assert (= e_301 (select a_300 i0)))
(assert (= e_303 (select a_300 i7)))
(assert (= e_306 (select a_282 i_305)))
(assert (= e_307 (select a_304 i_305)))
(assert (= i_305 (sk a_282 a_304)))
(assert (not (= e_306 e_307)))
(check-sat)
(exit)
