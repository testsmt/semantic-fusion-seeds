...
(assert (not (=> (forall ((n node$type)) (=> (or (= (m_cmd (select chan2 n)) inv) (= (m_cmd (select chan3 n)) invack)) (= (select shrset n) Truth)) ) (=> (= (m_cmd (select chan2 send_inv$i)) empty) (=> (= (select invset send_inv$i) Truth) (=> (or (= curcmd reqe) (and (= curcmd reqs) (= exgntd Truth))) (=> (= chan2$1 (store chan2 send_inv$i (c_msg$type inv (m_data (select chan2 send_inv$i))))) (=> (= invset$1 (store invset send_inv$i Falsity)) (forall ((n node$type)) (=> (or (= (m_cmd (select chan2$1 n)) inv) (= (m_cmd (select chan3 n)) invack)) (= (select shrset n) Truth)) )))))))))
(check-sat)
(exit)
