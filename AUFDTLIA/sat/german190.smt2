...
(assert (not (=> (and (and (forall ((n node$type)) (=> (or (= (m_cmd (select chan2 n)) inv) (= (m_cmd (select chan3 n)) invack)) (or (and (= curcmd reqs) (= exgntd Truth)) (= curcmd reqe))) ) (forall ((n node$type)) (=> (= (m_cmd (select chan3 n)) invack) (= (m_cmd (select chan2 n)) empty)) )) (forall ((n node$type)) (=> (or (= (m_cmd (select chan2 n)) inv) (= (m_cmd (select chan3 n)) invack)) (= (select shrset n) Truth)) )) (=> (= (m_cmd (select chan3 recv_invack$i)) invack) (=> (not (= curcmd empty)) (=> (= chan3$1 (store chan3 recv_invack$i (c_msg$type empty (m_data (select chan3 recv_invack$i))))) (=> (= shrset$1 (store shrset recv_invack$i Falsity)) (= (ite (= exgntd Truth) (ite (=> (= exgntd$1 Falsity) (=> (= memdata$1 (m_data (select chan3$1 recv_invack$i))) (forall ((n node$type)) (=> (or (= (m_cmd (select chan2 n)) inv) (= (m_cmd (select chan3$1 n)) invack)) (or (and (= curcmd reqs) (= exgntd$1 Truth)) (= curcmd reqe))) ))) Truth Falsity) (ite (forall ((n node$type)) (=> (or (= (m_cmd (select chan2 n)) inv) (= (m_cmd (select chan3$1 n)) invack)) (or (and (= curcmd reqs) (= exgntd Truth)) (= curcmd reqe))) ) Truth Falsity)) Truth))))))))
(check-sat)
(exit)
