...
(assert (not (=> (and (and (forall ((n node$type)) (=> (= (c_state (select cache n)) exclusive) (= exgntd Truth)) ) (forall ((n node$type)) (=> (= exgntd Truth) (= (select shrset n) (ite (= n auxnode) Truth Falsity))) )) (forall ((n node$type)) (=> (not (= (select shrset n) Truth)) (and (= (m_cmd (select chan2 n)) empty) (= (m_cmd (select chan3 n)) empty))) )) (=> (= (m_cmd (select chan3 recv_invack$i)) invack) (=> (not (= curcmd empty)) (=> (= chan3$1 (store chan3 recv_invack$i (c_msg$type empty (m_data (select chan3 recv_invack$i))))) (=> (= shrset$1 (store shrset recv_invack$i Falsity)) (= (ite (= exgntd Truth) (ite (=> (= exgntd$1 Falsity) (=> (= memdata$1 (m_data (select chan3$1 recv_invack$i))) (forall ((n node$type)) (=> (= (c_state (select cache n)) exclusive) (= exgntd$1 Truth)) ))) Truth Falsity) (ite (forall ((n node$type)) (=> (= (c_state (select cache n)) exclusive) (= exgntd Truth)) ) Truth Falsity)) Truth))))))))
(check-sat)
(exit)
