...
(assert (not (=> (forall ((n node$type)) (=> (or (= (m_cmd (select chan2 n)) inv) (= (m_cmd (select chan3 n)) invack)) (not (= (select invset n) Truth))) ) (=> (= (m_cmd (select chan2 send_invack$i)) inv) (=> (= (m_cmd (select chan3 send_invack$i)) empty) (=> (= chan2$1 (store chan2 send_invack$i (c_msg$type empty (m_data (select chan2 send_invack$i))))) (=> (= chan3$2 (store chan3 send_invack$i (c_msg$type invack (m_data (select chan3 send_invack$i))))) (= (ite (= (c_state (select cache send_invack$i)) exclusive) (ite (=> (= chan3$1 (store chan3$2 send_invack$i (c_msg$type (m_cmd (select chan3$2 send_invack$i)) (c_data (select cache send_invack$i))))) (=> (= cache$1 (store cache send_invack$i (c_cache$type invalid (c_data (select cache send_invack$i))))) (forall ((n node$type)) (=> (or (= (m_cmd (select chan2$1 n)) inv) (= (m_cmd (select chan3$1 n)) invack)) (not (= (select invset n) Truth))) ))) Truth Falsity) (ite (=> (= cache$1 (store cache send_invack$i (c_cache$type invalid (c_data (select cache send_invack$i))))) (forall ((n node$type)) (=> (or (= (m_cmd (select chan2$1 n)) inv) (= (m_cmd (select chan3$2 n)) invack)) (not (= (select invset n) Truth))) )) Truth Falsity)) Truth))))))))
(check-sat)
(exit)
