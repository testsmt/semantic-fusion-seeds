...
(assert (not (=> (forall ((n node$type)) (=> (= (m_cmd (select chan3 n)) invack) (= (m_cmd (select chan2 n)) empty)) ) (=> (= curcmd empty) (=> (= (m_cmd (select chan1 recv_req_exclusive$i)) reqe) (=> (= curcmd$1 reqe) (=> (= curptr$1 recv_req_exclusive$i) (=> (= chan1$1 (store chan1 recv_req_exclusive$i (c_msg$type empty (m_data (select chan1 recv_req_exclusive$i))))) (=> (= invset$1 shrset) (forall ((n node$type)) (=> (= (m_cmd (select chan3 n)) invack) (= (m_cmd (select chan2 n)) empty)) ))))))))))
(check-sat)
(exit)
