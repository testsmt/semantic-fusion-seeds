...
(assert (not (=> (forall ((n node$type)) (=> (= (m_cmd (select chan2 n)) gnte) (= exgntd Truth)) ) (=> (= (m_cmd (select chan2 recv_gnt_exclusive$i)) gnte) (=> (= cache$2 (store cache recv_gnt_exclusive$i (c_cache$type exclusive (c_data (select cache recv_gnt_exclusive$i))))) (=> (= cache$1 (store cache$2 recv_gnt_exclusive$i (c_cache$type (c_state (select cache$2 recv_gnt_exclusive$i)) (m_data (select chan2 recv_gnt_exclusive$i))))) (=> (= chan2$1 (store chan2 recv_gnt_exclusive$i (c_msg$type empty (m_data (select chan2 recv_gnt_exclusive$i))))) (forall ((n node$type)) (=> (= (m_cmd (select chan2$1 n)) gnte) (= exgntd Truth)) ))))))))
(check-sat)
(exit)
