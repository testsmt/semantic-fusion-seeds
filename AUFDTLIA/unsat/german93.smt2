...
(assert (not (=> (forall ((n node$type)) (=> (= (m_cmd (select chan3 n)) invack) (= (m_cmd (select chan2 n)) empty)) ) (=> (= (m_cmd (select chan1 send_req_shared$i)) empty) (=> (= (c_state (select cache send_req_shared$i)) invalid) (=> (= chan1$1 (store chan1 send_req_shared$i (c_msg$type reqs (m_data (select chan1 send_req_shared$i))))) (forall ((n node$type)) (=> (= (m_cmd (select chan3 n)) invack) (= (m_cmd (select chan2 n)) empty)) )))))))
(check-sat)
(exit)
