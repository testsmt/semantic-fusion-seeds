...
(assert (not (=> (forall ((n node$type)) (=> (= exgntd Truth) (= (select shrset n) (ite (= n auxnode) Truth Falsity))) ) (=> (= (c_state (select cache store_data$i)) exclusive) (=> (= cache$1 (store cache store_data$i (c_cache$type (c_state (select cache store_data$i)) store_data$d))) (=> (= auxdata$1 store_data$d) (forall ((n node$type)) (=> (= exgntd Truth) (= (select shrset n) (ite (= n auxnode) Truth Falsity))) )))))))
(check-sat)
(exit)
