...
(assert (not (=> (and (forall ((n node$type)) (=> (not (= (select shrset n) Truth)) (= (c_state (select cache n)) invalid)) ) (forall ((n node$type)) (=> (= (m_cmd (select chan3 n)) invack) (= (c_state (select cache n)) invalid)) )) (=> (= (m_cmd (select chan3 recv_invack$i)) invack) (=> (not (= curcmd empty)) (=> (= chan3$1 (store chan3 recv_invack$i (c_msg$type empty (m_data (select chan3 recv_invack$i))))) (=> (= shrset$1 (store shrset recv_invack$i Falsity)) (= (ite (= exgntd Truth) (ite (=> (= exgntd$1 Falsity) (=> (= memdata$1 (m_data (select chan3$1 recv_invack$i))) (forall ((n node$type)) (=> (not (= (select shrset$1 n) Truth)) (= (c_state (select cache n)) invalid)) ))) Truth Falsity) (ite (forall ((n node$type)) (=> (not (= (select shrset$1 n) Truth)) (= (c_state (select cache n)) invalid)) ) Truth Falsity)) Truth))))))))
(check-sat)
(exit)
