...
(assert (not (=> (forall ((i node$type) (j node$type)) (=> (and (not (= i j)) (= (c_state (select cache i)) exclusive)) (= (c_state (select cache j)) invalid)) ) (forall ((i node$type) (j node$type)) (=> (and (not (= i j)) (= (c_state (select cache i)) shared)) (or (= (c_state (select cache j)) invalid) (= (c_state (select cache j)) shared))) ))))
(check-sat)
(exit)
