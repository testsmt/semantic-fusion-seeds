...
(assert (not (=> (and (and (and (and (and (and (and (and (and (= auxdata dummy) (= memdata dummy)) (= curcmd empty)) (= exgntd Falsity)) (= shrset mk_array_6)) (= invset mk_array_5)) (= cache mk_array_4)) (= chan3 mk_array_3)) (= chan2 mk_array_2)) (= chan1 mk_array_1)) (forall ((n node$type)) (=> (or (= (m_cmd (select chan2 n)) inv) (= (m_cmd (select chan3 n)) invack)) (= (select shrset n) Truth)) ))))
(check-sat)
(exit)
