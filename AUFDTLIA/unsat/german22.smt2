...
(assert (not (=> (forall ((n node$type)) (=> (= exgntd Truth) (= (select shrset n) (ite (= n auxnode) Truth Falsity))) ) (=> (= curcmd reqs) (=> (not (= exgntd Truth)) (=> (= (m_cmd (select chan2 curptr)) empty) (=> (= chan2$1 (store chan2 curptr (c_msg$type gnts memdata))) (=> (= shrset$1 (store shrset curptr Truth)) (=> (= curcmd$1 empty) (forall ((n node$type)) (=> (= exgntd Truth) (= (select shrset$1 n) (ite (= n auxnode) Truth Falsity))) ))))))))))
(check-sat)
(exit)
