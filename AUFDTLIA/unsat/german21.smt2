...
(assert (not (=> (forall ((n node$type)) (=> (= exgntd Truth) (= (select shrset n) (ite (= n auxnode) Truth Falsity))) ) (=> (= (m_cmd (select chan3 recv_invack$i)) invack) (=> (not (= curcmd empty)) (=> (= chan3$1 (store chan3 recv_invack$i (c_msg$type empty (m_data (select chan3 recv_invack$i))))) (=> (= shrset$1 (store shrset recv_invack$i Falsity)) (= (ite (= exgntd Truth) (ite (=> (= exgntd$1 Falsity) (=> (= memdata$1 (m_data (select chan3$1 recv_invack$i))) (forall ((n node$type)) (=> (= exgntd$1 Truth) (= (select shrset$1 n) (ite (= n auxnode) Truth Falsity))) ))) Truth Falsity) (ite (forall ((n node$type)) (=> (= exgntd Truth) (= (select shrset$1 n) (ite (= n auxnode) Truth Falsity))) ) Truth Falsity)) Truth))))))))
(check-sat)
(exit)
