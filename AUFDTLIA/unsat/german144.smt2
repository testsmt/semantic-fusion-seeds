...
(assert (not (=> (and (and (forall ((n node$type)) (=> (= (c_state (select cache n)) exclusive) (= exgntd Truth)) ) (forall ((n node$type)) (=> (= exgntd Truth) (= (select shrset n) (ite (= n auxnode) Truth Falsity))) )) (forall ((n node$type)) (=> (not (= (select shrset n) Truth)) (= (c_state (select cache n)) invalid)) )) (forall ((i node$type) (j node$type)) (=> (and (not (= i j)) (= (c_state (select cache i)) exclusive)) (= (c_state (select cache j)) invalid)) ))))
(check-sat)
(exit)
