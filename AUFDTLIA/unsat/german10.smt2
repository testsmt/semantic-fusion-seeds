...
(assert (not (=> (forall ((n node$type)) (=> (= (c_state (select cache n)) exclusive) (= exgntd Truth)) ) (=> (= curcmd reqe) (=> (not (= exgntd Truth)) (=> (= (m_cmd (select chan2 curptr)) empty) (=> (forall ((j node$type)) (not (= (select shrset j) Truth)) ) (=> (= chan2$1 (store chan2 curptr (c_msg$type gnte memdata))) (=> (= shrset$1 (store shrset curptr Truth)) (=> (= curcmd$1 empty) (=> (= exgntd$1 Truth) (=> (= auxnode$1 curptr) (forall ((n node$type)) (=> (= (c_state (select cache n)) exclusive) (= exgntd$1 Truth)) )))))))))))))
(check-sat)
(exit)
