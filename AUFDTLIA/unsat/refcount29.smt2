...
(assert (not (=> (forall ((p process$type)) (=> (not (= (select ref p) null)) (= (S$mem p (select handles (select ref p))) Truth)) ) (=> (not (= destroy$r null)) (=> (= (select valid destroy$r) Truth) (=> (= (select count destroy$r) 0) (=> (= valid$1 (store valid destroy$r Falsity)) (forall ((p process$type)) (=> (not (= (select ref p) null)) (= (S$mem p (select handles (select ref p))) Truth)) ))))))))
(check-sat)
(exit)
