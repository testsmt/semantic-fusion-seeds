...
(declare-fun allocate$p () process$type)
(declare-fun allocate$r () resource$type)
...
(declare-fun ref$1 () (Array process$type resource$type))
(assert (not (=> (and (and (forall ((r resource$type)) (=> (and (not (= r null)) (= (select valid r) Truth)) (= (select count r) (S$cardinality (select handles r)))) ) (forall ((p process$type)) (=> (not (= (select ref p) null)) (= (S$mem p (select handles (select ref p))) Truth)) )) (forall ((p process$type) (r resource$type)) (=> (and (and (not (= r null)) (= (select valid r) Truth)) (= (S$mem p (select handles r)) Truth)) (= (select ref p) r)) )) (=> (= (select valid allocate$r) Truth) (=> (= (select ref allocate$p) null) (=> (= ref$1 (store ref allocate$p allocate$r)) (=> (= count$1 (store count allocate$r (+ (select count allocate$r) 1))) (=> (= handles$1 (store handles allocate$r (S$add allocate$p (select handles allocate$r)))) (forall ((r resource$type)) (=> (and (not (= r null)) (= (select valid r) Truth)) (= (select count$1 r) (S$cardinality (select handles$1 r)))) )))))))))
(check-sat)
(exit)
