...
(assert (not (=> (forall ((n node$type)) (=> (= (m_cmd (select chan2 n)) gnte) (= exgntd Truth)) ) (=> (= curcmd empty) (=> (= (m_cmd (select chan1 recv_req_shared$i)) reqs) (=> (= curcmd$1 reqs) (=> (= curptr$1 recv_req_shared$i) (=> (= chan1$1 (store chan1 recv_req_shared$i (c_msg$type empty (m_data (select chan1 recv_req_shared$i))))) (=> (= invset$1 shrset) (forall ((n node$type)) (=> (= (m_cmd (select chan2 n)) gnte) (= exgntd Truth)) ))))))))))
(check-sat)
(exit)
