...
(declare-fun auxnode$1 () node$type)
...
(assert (not (=> (and (forall ((n node$type)) (=> (or (= (m_cmd (select chan2 n)) inv) (= (m_cmd (select chan3 n)) invack)) (or (and (= curcmd reqs) (= exgntd Truth)) (= curcmd reqe))) ) (forall ((n node$type)) (=> (or (= (m_cmd (select chan2 n)) inv) (= (m_cmd (select chan3 n)) invack)) (= (select shrset n) Truth)) )) (=> (= curcmd reqe) (=> (not (= exgntd Truth)) (=> (= (m_cmd (select chan2 curptr)) empty) (=> (forall ((j node$type)) (not (= (select shrset j) Truth)) ) (=> (= chan2$1 (store chan2 curptr (c_msg$type gnte memdata))) (=> (= shrset$1 (store shrset curptr Truth)) (=> (= curcmd$1 empty) (=> (= exgntd$1 Truth) (=> (= auxnode$1 curptr) (forall ((n node$type)) (=> (or (= (m_cmd (select chan2$1 n)) inv) (= (m_cmd (select chan3 n)) invack)) (or (and (= curcmd$1 reqs) (= exgntd$1 Truth)) (= curcmd$1 reqe))) )))))))))))))
(check-sat)
(exit)
