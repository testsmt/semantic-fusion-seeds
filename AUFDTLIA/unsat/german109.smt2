...
(assert (not (=> (and (forall ((n node$type)) (=> (or (= (m_cmd (select chan2 n)) inv) (= (m_cmd (select chan3 n)) invack)) (not (= (select invset n) Truth))) ) (forall ((n node$type)) (=> (or (= (m_cmd (select chan2 n)) inv) (= (m_cmd (select chan3 n)) invack)) (or (and (= curcmd reqs) (= exgntd Truth)) (= curcmd reqe))) )) (=> (= curcmd empty) (=> (= (m_cmd (select chan1 recv_req_exclusive$i)) reqe) (=> (= curcmd$1 reqe) (=> (= curptr$1 recv_req_exclusive$i) (=> (= chan1$1 (store chan1 recv_req_exclusive$i (c_msg$type empty (m_data (select chan1 recv_req_exclusive$i))))) (=> (= invset$1 shrset) (forall ((n node$type)) (=> (or (= (m_cmd (select chan2 n)) inv) (= (m_cmd (select chan3 n)) invack)) (not (= (select invset$1 n) Truth))) ))))))))))
(check-sat)
(exit)
