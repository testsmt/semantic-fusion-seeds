...
(assert (not (=> (forall ((n node$type)) (=> (= (m_cmd (select chan3 n)) invack) (= (c_state (select cache n)) invalid)) ) (=> (= (c_state (select cache store_data$i)) exclusive) (=> (= cache$1 (store cache store_data$i (c_cache$type (c_state (select cache store_data$i)) store_data$d))) (=> (= auxdata$1 store_data$d) (forall ((n node$type)) (=> (= (m_cmd (select chan3 n)) invack) (= (c_state (select cache$1 n)) invalid)) )))))))
(check-sat)
(exit)
