...
(assert (not (=> (forall ((n node$type)) (=> (= (m_cmd (select chan3 n)) invack) (= (c_state (select cache n)) invalid)) ) (=> (= (m_cmd (select chan1 send_req_exclusive$i)) empty) (=> (not (= (c_state (select cache send_req_exclusive$i)) exclusive)) (=> (= chan1$1 (store chan1 send_req_exclusive$i (c_msg$type reqe (m_data (select chan1 send_req_exclusive$i))))) (forall ((n node$type)) (=> (= (m_cmd (select chan3 n)) invack) (= (c_state (select cache n)) invalid)) )))))))
(check-sat)
(exit)
