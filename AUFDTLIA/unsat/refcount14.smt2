...
(assert (not (=> (and (forall ((p process$type)) (=> (not (= (select ref p) null)) (= (S$mem p (select handles (select ref p))) Truth)) ) (forall ((p process$type)) (=> (not (= (select ref p) null)) (= (select valid (select ref p)) Truth)) )) (=> (= (select valid allocate$r) Truth) (=> (= (select ref allocate$p) null) (=> (= ref$1 (store ref allocate$p allocate$r)) (=> (= count$1 (store count allocate$r (+ (select count allocate$r) 1))) (=> (= handles$1 (store handles allocate$r (S$add allocate$p (select handles allocate$r)))) (forall ((p process$type)) (=> (not (= (select ref$1 p) null)) (= (S$mem p (select handles$1 (select ref$1 p))) Truth)) )))))))))
(check-sat)
(exit)
