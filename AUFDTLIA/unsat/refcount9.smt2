...
(assert (not (=> (forall ((p process$type) (r resource$type)) (=> (and (and (not (= r null)) (= (select valid r) Truth)) (= (S$mem p (select handles r)) Truth)) (= (select ref p) r)) ) (=> (= (select valid allocate$r) Truth) (=> (= (select ref allocate$p) null) (=> (= ref$1 (store ref allocate$p allocate$r)) (=> (= count$1 (store count allocate$r (+ (select count allocate$r) 1))) (=> (= handles$1 (store handles allocate$r (S$add allocate$p (select handles allocate$r)))) (forall ((p process$type) (r resource$type)) (=> (and (and (not (= r null)) (= (select valid r) Truth)) (= (S$mem p (select handles$1 r)) Truth)) (= (select ref$1 p) r)) )))))))))
(check-sat)
(exit)
