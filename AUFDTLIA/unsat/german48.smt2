...
(assert (not (=> (forall ((n node$type)) (=> (= (m_cmd (select chan2 n)) gnte) (= exgntd Truth)) ) (=> (= curcmd reqs) (=> (not (= exgntd Truth)) (=> (= (m_cmd (select chan2 curptr)) empty) (=> (= chan2$1 (store chan2 curptr (c_msg$type gnts memdata))) (=> (= shrset$1 (store shrset curptr Truth)) (=> (= curcmd$1 empty) (forall ((n node$type)) (=> (= (m_cmd (select chan2$1 n)) gnte) (= exgntd Truth)) ))))))))))
(check-sat)
(exit)
