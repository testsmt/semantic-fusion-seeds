...
(assert (not (=> (forall ((p process$type) (r resource$type)) (=> (and (and (not (= r null)) (= (select valid r) Truth)) (= (S$mem p (select handles r)) Truth)) (= (select ref p) r)) ) (=> (not (= create$r null)) (=> (not (= (select valid create$r) Truth)) (=> (= valid$1 (store valid create$r Truth)) (=> (= count$1 (store count create$r 0)) (=> (= handles$1 (store handles create$r S$empty)) (forall ((p process$type) (r resource$type)) (=> (and (and (not (= r null)) (= (select valid$1 r) Truth)) (= (S$mem p (select handles$1 r)) Truth)) (= (select ref p) r)) )))))))))
(check-sat)
(exit)
