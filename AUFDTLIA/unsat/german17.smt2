...
(assert (not (=> (forall ((n node$type)) (=> (= exgntd Truth) (= (select shrset n) (ite (= n auxnode) Truth Falsity))) ) (=> (= curcmd empty) (=> (= (m_cmd (select chan1 recv_req_shared$i)) reqs) (=> (= curcmd$1 reqs) (=> (= curptr$1 recv_req_shared$i) (=> (= chan1$1 (store chan1 recv_req_shared$i (c_msg$type empty (m_data (select chan1 recv_req_shared$i))))) (=> (= invset$1 shrset) (forall ((n node$type)) (=> (= exgntd Truth) (= (select shrset n) (ite (= n auxnode) Truth Falsity))) ))))))))))
(check-sat)
(exit)
