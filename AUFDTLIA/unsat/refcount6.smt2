...
(declare-fun mk_array_1 () (Array resource$type BOOL))
(assert (forall ((mk_array_1_index resource$type)) (= (select mk_array_1 mk_array_1_index) Falsity) ))
(declare-fun mk_array_2 () (Array process$type resource$type))
(assert (forall ((mk_array_2_index process$type)) (= (select mk_array_2 mk_array_2_index) null) ))
(assert (not (=> (and (= ref mk_array_2) (= valid mk_array_1)) (forall ((p process$type) (r resource$type)) (=> (and (and (not (= r null)) (= (select valid r) Truth)) (= (S$mem p (select handles r)) Truth)) (= (select ref p) r)) ))))
(check-sat)
(exit)
