...
(assert (not (=> (and (forall ((n node$type)) (=> (= (m_cmd (select chan3 n)) invack) (= (c_state (select cache n)) invalid)) ) (forall ((n node$type)) (=> (= (m_cmd (select chan3 n)) invack) (= (m_cmd (select chan2 n)) empty)) )) (=> (= (m_cmd (select chan2 recv_gnt_shared$i)) gnts) (=> (= cache$2 (store cache recv_gnt_shared$i (c_cache$type shared (c_data (select cache recv_gnt_shared$i))))) (=> (= cache$1 (store cache$2 recv_gnt_shared$i (c_cache$type (c_state (select cache$2 recv_gnt_shared$i)) (m_data (select chan2 recv_gnt_shared$i))))) (=> (= chan2$1 (store chan2 recv_gnt_shared$i (c_msg$type empty (m_data (select chan2 recv_gnt_shared$i))))) (forall ((n node$type)) (=> (= (m_cmd (select chan3 n)) invack) (= (c_state (select cache$1 n)) invalid)) ))))))))
(check-sat)
(exit)
