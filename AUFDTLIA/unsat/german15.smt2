...
(assert (not (=> (forall ((n node$type)) (=> (= exgntd Truth) (= (select shrset n) (ite (= n auxnode) Truth Falsity))) ) (=> (= (m_cmd (select chan1 send_req_shared$i)) empty) (=> (= (c_state (select cache send_req_shared$i)) invalid) (=> (= chan1$1 (store chan1 send_req_shared$i (c_msg$type reqs (m_data (select chan1 send_req_shared$i))))) (forall ((n node$type)) (=> (= exgntd Truth) (= (select shrset n) (ite (= n auxnode) Truth Falsity))) )))))))
(check-sat)
(exit)
